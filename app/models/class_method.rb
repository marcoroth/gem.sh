...
class ClassMethod < MethodDefinition
  def url(gem)
    if target.is_a?(ModuleDefinition)
      ...
      Router.gem_version_class_class_method_path(gem.name, gem.version, target.qualified_name, name)
    else
      Router.gem_version_class_method_path(gem.name, gem.version, name)
    end
  end
